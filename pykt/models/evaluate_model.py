...
def evaluate(model, test_loader, model_name, rel=None, save_path="", save_io_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            # 新增：如果STD_OUTPUT为1，计算当前batch的AUC
            if std_output == 1:
                try:
                    # 检查当前batch是否有足够的正负样本来计算AUC
                    if len(np.unique(t_valid)) > 1 and len(t_valid) > 0:
                        batch_auc = metrics.roc_auc_score(y_true=t_valid, y_score=y_valid)
                        batch_aucs.append(batch_auc)
                        print(f"Batch {len(batch_aucs)}: AUC = {batch_auc:.4f}, samples = {len(t_valid)}")
                    else:
                        print(f"Batch {len(batch_aucs)+1}: 跳过AUC计算 (标签类别不足或样本为空)")
                except Exception as e:
                    print(f"Batch {len(batch_aucs)+1}: AUC计算出错 - {e}")
        ...
        # 新增：如果STD_OUTPUT为1，计算AUC的标准差
        if std_output == 1 and len(batch_aucs) > 0:
            auc_std = np.std(batch_aucs, ddof=1)  # 使用样本标准差
            auc_mean = np.mean(batch_aucs)
            print(f"批次AUC统计信息:")
            print(f"  有效批次数量: {len(batch_aucs)}")
            print(f"  AUC均值: {auc_mean:.4f}")
            print(f"  AUC标准差: {auc_std:.4f}")
            print(f"  AUC范围: [{min(batch_aucs):.4f}, {max(batch_aucs):.4f}]")
        else:
            auc_std = 0.0
            print("未计算批次AUC标准差 (STD_OUTPUT != 1 或无有效批次)")
